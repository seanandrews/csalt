...
H0 = np.sqrt(sc.k * Tmid0 / (2.37 * (sc.m_p + sc.m_e))) / \
    np.sqrt(sc.G * mstar * 1.989e30 / (10 * sc.au)**3) / sc.au
print(H0)
hs_T = 2 * (H0 / 10.)
ws_T = 0.1 * hs_T
...
pars  = np.array([incl, PA, mstar, r_l, Tmid0, Tatm0, qmid, qatm, hs_T, ws_T,
                  Sigma0_gas, p1, p2, xmol, depl, Tfrz, zrmax, rmin, rmax, xi,
                  Vsys, dx, dy])

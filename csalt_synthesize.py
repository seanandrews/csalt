...
from csalt_models import vismodel_full as vm_full
sys.path.append('configs_synth/')


### Ingest the configuration file (in configs_synth/). 
if len(sys.argv) == 1:
    print('\nSpecify a configuration filename in configs_synth/ as an '+ \
          'argument: e.g., \n')
    print('        python csalt_synthesize.py <config_filename>\n')
    sys.exit()
else:
...
for EB in range(len(inp.template)):
    ...
    os.system('casa --nologger --logfile CASA_logs/'+ \
              'gen_template.'+tmp_+'.log ' + \
              '-c CASA_scripts/gen_template.py '+config_filename+' '+str(EB))

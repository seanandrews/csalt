...
# cube files
kdir = inp.reduced_dir+'sg_'+mdl+'_M15/images/'
pdir = inp.reduced_dir+'sg_'+mdl+'_M05/images/'
adir = inp.reduced_dir+'sg_'+mdl+'/images/'
cfiles = [kdir+'sg_'+mdl+'_M15_pure.DATA.image.fits',
          adir+'sg_'+mdl+'_pure.DATA.image.fits',
          pdir+'sg_'+mdl+'_M05_pure.DATA.image.fits']
...
# display properties
vmin, vmax = 0, 70
nchans, ch0, dch = 6, 74, 1
xlims = np.array([2.1, -2.1])
cm = cm.get_cmap('cmr.eclipse', 50)
norm = ImageNormalize(vmin=vmin, vmax=vmax, stretch=AsinhStretch())

fig, axs = plt.subplots(nrows=3, ncols=nchans, figsize=(7.5, 3.6))
...
# colorbar
cbax = fig.add_axes([0.925, 0.138, 0.015, 0.807])
...
fig.savefig('figs/chanmap_Mdisk_demos.pdf')
fig.clf()

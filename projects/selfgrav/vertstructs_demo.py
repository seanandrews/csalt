...
# configuration files
cfgs = ['gen_sg_sharp', 'gen_sg_sharp', 'gen_sg_sharp']
lbls = ['$\mathsf{sharp \,\, a}$', '$\mathsf{sharp \,\, b}$', 
        '$\mathsf{sharp \,\, c}$']
...
for i in range(len(cfgs)):
    ...
    if i == 0:
        Tgas = temperature(rr, zz, inp, iso=True)
    else:
    ...
    if i == 0:
        ngas = numberdensity_iso(rr, zz, inp)
    elif i == 2:
        ngas = numberdensity(rr, zz, inp, selfgrav=True)
    else:
        ngas = numberdensity(rr, zz, inp)

    # plot the gas densities
    above = (ngas <= 1e2)
    imn = axs[1, i].imshow(np.log10(ngas), origin='lower', cmap=cmap_n, 
                           extent=ext, aspect='auto', 
                           vmin=n_ra[0], vmax=n_ra[1])


    # compute the pressure support velocity residuals
    om_k = omega_kep(rr, zz, inp)
    if i == 0:
        om_prs2 = eps_P(rr, zz, inp, niso=True)
    elif i == 2:
        om_prs2 = eps_P(rr, zz, inp, nselfgrav=True)
    else:
        om_prs2 = eps_P(rr, zz, inp)
...
cb_n = Colorbar(ax=cbax_n, mappable=imn, orientation='vertical',
                ticklocation='right', ticks=[5, 6, 7, 8, 9, 10, 11, 12])
...
fig.savefig('figs/vertstructs_demo.pdf')
fig.clf()

...
print(tmp_weights.shape, tmp_uvw.shape)
...
# Number of visibilities per timestamp
nstamps = v_LSRK.shape[0]
nperstamp = np.int(nvis / nstamps)
print(nstamps, nperstamp)
...
clean_vis_out = clean_vis_SRF[:,::in_.spec_oversample,:,:]
freq_TOPO_out = tmp['freq_TOPO'][::in_.spec_oversample]
freq_LSRK_out = tmp['freq_LSRK'][:,::in_.spec_oversample]

print(clean_vis_out.shape, freq_TOPO_out.shape, freq_LSRK_out.shape)
